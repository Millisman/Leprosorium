 ...
 *       https://github.com/simonsj/libssh/tree/master/examples
 * TODO: ipv6
...
static int handle_ssh_auth(ssh_session session) {
    ...
    if (json_logging_file /*|| json_logging_server*/) {
        json_log_session(ip, banner_c, banner_s, kex_algo, cipher_in, cipher_out, hmac_in, hmac_out);
    }

    if (verbose) {
        log_entry("Session:  %s|%s|%s|%s|%s|%s|%s",
            banner_c, banner_s, kex_algo, cipher_in, cipher_out, hmac_in, hmac_out);
    }
    ...
    for (;;) {
        ...
        if (message != NULL) {
            ...
            switch (ssh_message_subtype(message)) {
                ...
                // SSH_AUTH_METHOD_PUBLICKEY -- TODO
            ...
            ssh_message_reply_default(message); // ssh_message_service_reply_success
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "vh?p:dLl:a:b:r:f:su:j:"/*J:P:*/)) != -1) {
        switch (opt) {
        case 'p': /* Listen port */
            port = atoi(optarg);
            break;
