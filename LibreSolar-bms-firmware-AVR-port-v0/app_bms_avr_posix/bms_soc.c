...
void bms_soc_update(Bms *bms)
{
    ...
    uint32_t now = millis2(); // Get system uptime. This routine returns the elapsed time since the system booted, in milliseconds.

    coulomb_counter_mAs += bms->status.pack_current * (abs(now - last_update));
