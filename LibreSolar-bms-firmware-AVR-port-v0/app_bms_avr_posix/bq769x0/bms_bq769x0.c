...
{
    long idle_secs = uptime2() - bms->status.no_idle_timestamp;
...
int bms_apply_dis_scp(Bms *bms)
{
    ...
    protect1.flags.RSNS = 1;

    protect1.flags.SCD_THRESH = 0;
    ...
    bms->conf.dis_sc_limit = SCD_threshold_setting[protect1.flags.SCD_THRESH] / bms->conf.shunt_res_mOhm;
    bms->conf.dis_sc_delay_us = SCD_delay_setting[protect1.flags.SCD_DELAY];
...
int bms_apply_dis_ocp(Bms *bms)
{
    ...
    // store actually configured values
    bms->conf.dis_oc_limit = OCD_threshold_setting[protect2.flags.OCD_THRESH] / bms->conf.shunt_res_mOhm;
    bms->conf.dis_oc_delay_ms = OCD_delay_setting[protect2.flags.OCD_DELAY];
...
int bms_apply_cell_uvp(Bms *bms)
{
    ...
    bms->conf.cell_uv_delay_ms = UV_delay_setting[protect3.flags.UV_DELAY];
...
int bms_apply_cell_ovp(Bms *bms)
{
    ...
    bms->conf.cell_ov_delay_ms = OV_delay_setting[protect3.flags.OV_DELAY];
...
void bms_read_current(Bms *bms)
{
    ...
    if (sys_stat.flags.CC_READY == 1) {
        ...
        if (fabs(bms->status.pack_current) > bms->conf.bal_idle_current) {
            bms->status.no_idle_timestamp = uptime2();
        }
...
void bms_handle_errors(Bms *bms)
{
    ...
    if (!bq769x0_alert_flag() && error_status == 0) {
        ...
        unsigned int sec_since_interrupt = uptime2() - bq769x0_alert_timestamp();
